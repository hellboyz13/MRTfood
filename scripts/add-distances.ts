/**
 * Script to add distance_to_station, lat, lng columns to food_listings
 * and populate them using Google Places API
 */
...
async function searchPlace(restaurantName: string): Promise<{ lat: number; lng: number } | null> {
...
async function addDistances() {
  ...
  for (const listing of listings || []) {
    console.log(`[${updated + failed + 1}] Processing: ${listing.name}...`);

    // Get coordinates from Google Places
    const coords = await searchPlace(listing.name);
    ...
    if (listing.station_id && stationCoordinates[listing.station_id]) {
      ...
      distance = Math.round(haversineDistance(coords.lat, coords.lng, station.lat, station.lng));
      console.log(`  - Distance to ${station.name}: ${distance}m`);
    }
...
addDistances().catch(console.error);
