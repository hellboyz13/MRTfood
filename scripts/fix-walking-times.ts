/**
 * Script to fix walking distances using Google Directions API
 * Gets real walking distance and time (not straight-line)
 */
...
async function fixWalkingTimes() {
  ...
  console.log(`Found ${listings?.length || 0} listings with coordinates\n`);

  let updated = 0;
  let failed = 0;
  const comparisons: { name: string; before: string; after: string }[] = [];

  for (const listing of listings || []) {
    if (!listing.station_id || !stationCoordinates[listing.station_id]) {
      console.log(`[SKIP] ${listing.name} - no valid station`);
      failed++;
      continue;
    }

    const station = stationCoordinates[listing.station_id];
    const oldDistance = listing.distance_to_station;
    const oldTime = oldDistance ? Math.round(oldDistance / 80) : null;

    console.log(`[${updated + failed + 1}] ${listing.name}...`);
    ...
    const newDistance = walkingInfo.distance;
    const newTime = Math.round(walkingInfo.duration / 60); // Convert to minutes

    // Store comparison for output
    if (oldDistance) {
      comparisons.push({
        name: listing.name,
        before: `${oldDistance}m, ${oldTime} min (straight-line)`,
        after: `${newDistance}m, ${newTime} min (actual walking)`
      });
    }

    console.log(`  Before: ${oldDistance}m, ${oldTime} min | After: ${newDistance}m, ${newTime} min`);

    // Update the listing
    const { error: updateError } = await supabase
      .from('food_listings')
      .update({
        distance_to_station: newDistance,
        walking_time: walkingInfo.duration, // Store in seconds
      })
      .eq('id', listing.id);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log('BEFORE/AFTER COMPARISON (first 10)');
  console.log('='.repeat(60));
...
fixWalkingTimes().catch(console.error);
